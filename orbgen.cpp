...
EX bool buildPrizeMirror(cell *c, int freq) {
  ...
  if(freq && hrand(freq * 100 / orbprizefun(ap::getVirtualTreasureCount(ap::landChecksReceived[itShard]))) >= 100)
    return false;
...
EX void placePrizeOrb(cell *c) {
  ...
  if(hrandf() < 0.1f && ap::settings::hintOrb){
    ...
    if((ap::landProgressChecksSent[it] <= ap::progressCheck::orbunlocked && items[it] >= 10 && ap::settings::requiredTreasures >= 25) 
    | (ap::landProgressChecksSent[it] <= ap::progressCheck::orbunlockedglobal && items[it] >= 25 && ap::settings::requiredTreasures >= 50)) {
...
EX void placeLocalOrbs(cell *c) {
  ...
  for(auto& oi: orbinfos) {
    ...
    int tc = ap::getVirtualTreasureCount(ap::landChecksReceived[locTreas]);
...
EX void placeLocalSpecial(cell *c, int outof, int loc IS(1), int priz IS(1)) {
  ...
  int i = hrand(outof);
  if(i < loc && ap::getVirtualTreasureCount(ap::landChecksReceived[linf[(c->land)].treasure]) >= treasureForLocal() && !inv::on)
...
EX void placeCrossroadOrbs(cell *c) {
  ...
  for(auto& oi: orbinfos) {
    ...
    int treas = ap::getVirtualTreasureCount(ap::landChecksReceived[linf[oi.l].treasure]);
    int mintreas = 10;
